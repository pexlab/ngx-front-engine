...
import { capitalizedSpaceCase, kebabCase } from '../utils/string.utils';
import { mergeObj } from '../utils/type.utils';
import { rem } from '../utils/unit.utils';
...
export class ThemeService {
    ...
    public applyPalette( palette: HexColorRegister, element: HTMLElement, global = false ): void {
        ...
        if ( element === document.documentElement ) {
            ...
            let documentStyle = document.getElementById( 'fe-global-theme-palette' ) as HTMLStyleElement;

            if ( !documentStyle ) {
                documentStyle    = document.createElement( 'style' );
                documentStyle.id = 'fe-global-theme-palette';
                document.head.appendChild( documentStyle );
            }

            let stylesheet = '/* FrontEngine ' + ( global ? 'Global' : 'Local' ) + ' Theme Palette Stylesheet */';

            /* Extract every possible key/path from the palette */
            const elements = _.keysDeep( palette, { leavesOnly: true } );

            /* Iterate through every key/path and get its value */
            elements.forEach( ( pathKey ) => {
                ...
                if ( value && typeof value === 'string' ) {
                    ...
                    stylesheet += '\n\n';
                    stylesheet += '/* ' + capitalizedSpaceCase( key ) + ' */';
                    stylesheet += '\n';
                    stylesheet += ':root {';
                    stylesheet += '\n';
                    stylesheet += '\t' + propertyName + '-hex: ' + color.hex + ';';
                    stylesheet += '\n';
                    stylesheet += '\t' + propertyName + '-rgb: ' + color.channels.red + ', ' + color.channels.green + ', ' + color.channels.blue + ';';
                    stylesheet += '\n';
                    stylesheet += '\t' + propertyName + '-rgba: ' + color.channels.red + ', ' + color.channels.green + ', ' + color.channels.blue + ', ' + color.channels.alpha + ';';
                    stylesheet += '\n';
                    stylesheet += '\t' + propertyName + '-contrast-hex: ' + color.legible_contrast.hex + ';';
                    stylesheet += '\n';
                    stylesheet += '\t' + propertyName + '-contrast-rgb: ' + color.legible_contrast.channels.red + ', ' + color.legible_contrast.channels.green + ', ' + color.legible_contrast.channels.blue + ';';
                    stylesheet += '\n';
                    stylesheet += '\t' + propertyName + '-contrast-rgba: ' + color.legible_contrast.channels.red + ', ' + color.legible_contrast.channels.green + ', ' + color.legible_contrast.channels.blue + ', ' + color.legible_contrast.channels.alpha + ';';
                    stylesheet += '\n';
                    stylesheet += '}';
                }
            } );

            documentStyle.innerHTML = stylesheet;
    ...
    public applyTypography( typography: Typography, element: HTMLElement, global = false ): void {
        ...
        if ( element === document.documentElement ) {
            ...
            let documentStyle = document.getElementById( 'fe-global-theme-typography' ) as HTMLStyleElement;

            if ( !documentStyle ) {
                documentStyle    = document.createElement( 'style' );
                documentStyle.id = 'fe-global-theme-typography';
                document.head.appendChild( documentStyle );
            }

            let stylesheet = '/* FrontEngine ' + ( global ? 'Global' : 'Local' ) + ' Theme Typography Stylesheet */';
            ...
            Object.entries( typography ).forEach( ( font ) => {
                ...
                stylesheet += '\n\n';
                stylesheet += '/* ' + capitalizedSpaceCase( font[ 0 ] ) + ' */';
                stylesheet += '\n';
                stylesheet += ':root {';
                stylesheet += '\n';
                stylesheet += '\t' + propertyName + '-family: "' + font[ 1 ].name + '";';
                stylesheet += '\n';
                stylesheet += '\t' + propertyName + '-size: ' + (
                    isNaN( +font[ 1 ].size ) ?
                    font[ 1 ].size :
                    rem( +font[ 1 ].size )
                ) + ';';
                stylesheet += '\n';
                stylesheet += '\t' + propertyName + '-weight: ' + font[ 1 ].weight + ';';
                stylesheet += '\n';
                stylesheet += '}';
            } );
